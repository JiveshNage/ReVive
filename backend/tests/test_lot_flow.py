...
def test_lot_payment_can_be_marked_complete():
    ...
    accept_response = client.post(f"/api/offers/{offer_response.json()['id']}/accept")
    assert accept_response.status_code == 200, accept_response.text

    payment_response = client.post(f"/api/lots/{lot_id}/payment")
...
def test_full_traceability_lifecycle_and_certificate():
    ...
    # 3. Accept Offer
    accept_res = client.post(f"/api/offers/{offer_id}/accept")
    ...
    # 5. Payment
    payment_res = client.post(f"/api/lots/{lot_id}/payment")
...
def test_handover_validation_errors():
    ...
    # Mismatched collector error (collector 99 vs lot owner 1)
...
def test_safety_guidance_endpoint():
    ...
    # Check cable burning hazard slogan in 3 languages
...
def test_admin_anomalies_and_resolution():
    ...
    lot_res = client.post("/api/lots", json={"collector_id": 1, "material_id": 1, "quantity_kg": 10.0})
    assert lot_res.status_code == 200
    lot_id = lot_res.json()["id"]

    # 2. Create handover with significant discrepancy (4 kg instead of 10 kg -> 60% error)
    handover_res = client.post("/api/handover", json={
        "lot_id": lot_id,
        "collector_id": 1,
        "recycler_id": 1,
        "final_weight_kg": 4.0,
    ...
    anom_res = client.get("/api/admin/anomalies")
    ...
    resolve_res = client.post(f"/api/admin/anomalies/{wt_anom['id']}/resolve")
    assert resolve_res.status_code == 200
    assert resolve_res.json()["status"] == "resolved"

    # 5. Check status is now resolved
    anom_res2 = client.get("/api/admin/anomalies")
...
def test_recycler_verification_toggle():
    # Toggle off
    res_off = client.post("/api/recyclers/1/verify", json={"verified": False})
    assert res_off.status_code == 200
    assert res_off.json()["verified"] is False

    # Toggle on
    res_on = client.post("/api/recyclers/1/verify", json={"verified": True})
